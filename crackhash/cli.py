...
def show_banner():
    print((Colors.PURPLE) + (Colors.BOLD) + "   ___               _    _               _      ")
    print("  / __\ __ __ _  ___| | _| |__   __ _ ___| |__   ")           
    print(" / / | '__/ _` |/ __| |/ / '_ \ / _` / __| '_ \  ")         
    print("/ /__| | | (_| | (__|   <| | | | (_| \__ \ | | | ")       
    print("\____/_|  \__,_|\___|_|\_\_| |_|\__,_|___/_| |_| \n" + (Colors.ENDC))
    print("        --[    Version: " + (Colors.YELLOW) + (Colors.BOLD) + (__version__) + (Colors.ENDC) + "      ]--")
    print("        --[     Author: " + (Colors.CYAN) + (Colors.BOLD) + (__author__) + (Colors.ENDC) + "  ]--\n\n")
...
@click.option('--proxy-pass', is_eager=True, help='Specify a proxy user\'s password')
@click.option('-a', '--algo', type=click.Choice(ALGORITHMS), help='Specify hash\' algorithm')
@click.option('-h', '--hash', help='Specify a hash to crack')
@click.option('-f', '--file', type=click.Path(exists=True), help='Specify file containing hashes, formatted this way: hashtype:hashvalue')
def main(use_proxy, proxy_host, proxy_port, proxy_user, proxy_pass, algo, hash, file):
    ...
    if (hash == False) and (file == False):
        ...
        return 0
    if hash and file:
        print_error("Can't set both options at once. Choose between 'hash' or 'file' argument!")
        return 0
    ...
    if hash and (algo is None):
        ...
        print_info('The hash you are looking for probably is %s%s%s' % (Colors.BOLD, algo.upper(), Colors.ENDC))
